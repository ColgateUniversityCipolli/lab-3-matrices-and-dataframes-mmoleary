...
ggplot() +
  ...
  labs(x = "x", y = "y")
```
Unlike in Question 1, the parallelogram made in question 2 has 2 longer side lengths and thus a greater area. 
...
ggplot() +
  ...
  labs(x = "x", y = "y")
```
This parallelogram is also a rhombus with the same side lengths as the longer side in question 1. 
...
```{r}
leaning.ladder = data.frame(x=T[, 1],
                            y=T[, 2], 
                            group = group)
```
